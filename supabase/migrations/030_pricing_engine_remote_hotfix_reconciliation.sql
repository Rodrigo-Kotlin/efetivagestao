-- Migration 030: Pricing Engine Remote Hotfix Reconciliation
-- ============================================================
-- Reconciles post-029 hotfixes applied directly to the remote database.
-- All changes below were already running in production; this migration
-- converges a clean 001→030 database to match the remote definitions.
-- ============================================================

-- ──────────────────────────────────────────────────────────────
-- 1. fn_resolve_pricing_policy
-- Hotfix: deterministic tiebreaker using pv.created_at DESC
--   in all three scope resolution ORDER BY clauses
--   (catalog_item, category, default).
-- Original 029 had only pv.version_number DESC.
-- ──────────────────────────────────────────────────────────────
...
BEGIN
  ...
  -- CATALOG_ITEM scope (highest precedence)
  RETURN QUERY
  SELECT
  ...
  -- DEFAULT scope (lowest precedence)
  RETURN QUERY
  SELECT
    pp.id, pp.code, pp.name, pp.scope_type,
    pv.id, pv.version_number, pv.valid_from, pv.valid_to,
...
-- ──────────────────────────────────────────────────────────────
-- 2. fn_calculate_price
-- Hotfix: replace invalid text[] || 'literal' operations
--   with array_append() for violations and warnings arrays.
--   9 occurrences fixed (6 violations, 2 warnings, 1 early-return).
-- ──────────────────────────────────────────────────────────────
...
BEGIN
  ...
  IF p_cost_status IS NULL OR p_cost_status != 'CONFIRMED' THEN
    RETURN jsonb_build_object(
      ...
      'violations', to_jsonb(array_append(v_violations, 'COST_NOT_CONFIRMED')),
  ...
  ELSIF p_pricing_method = 'target_margin' THEN
    IF p_target_margin_rate >= 1 OR p_target_margin_rate < 0 THEN
      RETURN jsonb_build_object(
        ...
        'violations', to_jsonb(array_append(v_violations, 'INVALID_MARGIN')),
        'provenance', jsonb_build_object(
          'organization_id', p_organization_id,
          'pricing_policy_id', p_pricing_policy_id,
  ...
  ELSE
    RETURN jsonb_build_object(
      ...
      'violations', to_jsonb(array_append(v_violations, 'INVALID_METHOD'))
    );
  END IF;
  ...
  ELSE
    IF p_discount_rate < 0 OR p_discount_rate > 1 THEN
      RETURN jsonb_build_object(
        ...
        'violations', to_jsonb(array_append(v_violations, 'INVALID_DISCOUNT'))
      );
    END IF;
    ...
    IF p_maximum_discount_rate IS NOT NULL AND p_discount_rate > p_maximum_discount_rate THEN
      v_violations := array_append(v_violations, 'DISCOUNT_EXCEEDS_LIMIT');
    END IF;
  END IF;
  ...
  ELSE
    ...
    IF v_total_cost > 0 THEN
      v_warnings := array_append(v_warnings, 'ZERO_COST_DENOMINATOR');
    END IF;
  END IF;
  ...
  IF v_effective_price < v_total_cost THEN
    v_violations := array_append(v_violations, 'BELOW_COST');
  END IF;
  ...
  IF p_minimum_margin_rate IS NOT NULL AND v_effective_price > 0 THEN
    ...
    IF v_effective_price < v_floor_price THEN
      v_violations := array_append(v_violations, 'BELOW_MINIMUM_MARGIN');
    END IF;
  END IF;
...
-- ──────────────────────────────────────────────────────────────
-- 3. fn_simulate_price
-- Hotfix: SELECT * INTO v_cost_result pattern with proper
--   resolution_status field access for cost validation.
-- Original 029 used a multi-field SELECT INTO with individual
--   variables, missing resolution_status entirely.
-- ──────────────────────────────────────────────────────────────
...
BEGIN
  ...
  -- Cost resolution: SELECT * INTO to capture resolution_status
  SELECT *
  INTO v_cost_result
  ...
  v_cost_status     := COALESCE(v_cost_result.resolution_status, 'COST_NOT_CONFIRMED');
  v_cost_version_id := v_cost_result.version_id;
  v_cost_version_num := v_cost_result.version_number;
  v_cost_valid_from := v_cost_result.valid_from;
  v_cost_valid_to   := v_cost_result.valid_to;
  v_cost_table_id   := v_cost_result.cost_table_id;
